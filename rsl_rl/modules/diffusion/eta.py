...
Ported from dppo/model/diffusion/eta.py.
...
class EtaFixed(torch.nn.Module):
    ...
    def __call__(self, cond=None, batch_size=None, device=None):
        """Match input batch size, but do not depend on input.

        Accepts either:
          - cond: dict with key "state" or "rgb" (original dppo interface)
          - batch_size + device: scalar args (rsl_rl interface when no cond dict)
        """
        if cond is not None:
